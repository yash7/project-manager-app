...
public class App {
	...
	private static void sampleData( Connection c, ApplicationContext appCtx ) {
		...
		if ( activityService.getActivities().isEmpty() ) {
			activityService.addActivity(new Activity( null, project.getId(), Status.NEW, 0, 0, "The cool activity", 0, 0, 0, 0, "This activity is for cool people to do.", 0));
			activityService.addActivity(new Activity( null, project.getId(), Status.NEW, 0, 0, "The ugly activity", 0, 0, 0, 0, "This activity that no one wants to do.", 0 ));
		}
